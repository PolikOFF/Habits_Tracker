...
class PleasantHabitInRelatedHabitValidator:
    """
    Класс валидации того,
    что в связанные привычки могут попадать только привычки
...
class PleasantHabitNotHaveRelatedRewardValidator:
    """
    Класс валидации того,
    что у приятной привычки не может быть награды
...
class PeriodicityHabitValidator:
    """
    Класс валидации периодичности выполнения привычки.
    Не менее 1 раза в неделю и не более 7 раз в неделю.
    """
    ...
    def __call__(self, value):
        if self.periodicity in value:
            ...
            if not 1 <= periodicity <= 7:
                raise ValidationError(
                    'За одну неделю необходимо выполнить привычку '
                    'хотя бы один раз.')
